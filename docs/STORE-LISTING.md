...
Generate the icon at 128px with `npm run icons`.

---

## 4. Before submitting

- [ ] `npm run check` passes and `npm run package` produces `sf-releaselens.zip`.
- [ ] The version in `src/manifest.json` matches the release tag.
- [ ] Someone has actually loaded the packaged zip in Chrome and worked through
      `QA-CHECKLIST.md` §1–§7. **Not yet done** — see that document.
- [ ] The org connection has been exercised against a real Salesforce org.
      **Not yet done** — see the "requires a live org" section of the checklist.
- [ ] A privacy policy is published at a stable URL.
- [ ] The repository is public, so a reviewer can read the source that the
      permission justifications describe.
