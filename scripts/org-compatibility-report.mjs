...
async function measure(alias) {
  ...
  const row = {
    alias,
    kind: org.isScratch === true ? 'scratch' : org.isSandbox === true ? 'sandbox' : 'other',
  ...
  if (Array.isArray(deploys)) {
    ...
    for (const deploy of deploys) {
      ...
      if (deploy.CheckOnly === true) row.checkOnly += 1;
  ...
  row.deployTotal = await attempt(async () => {
    const page = await connection.toolingQuery('SELECT COUNT() FROM DeployRequest');
    return String(page.totalSize);
  });
...
async function main() {
  ...
  const table = [
    ...
    ...rows.map(
      (row) =>
        `| \`${row.alias}\` | ${row.kind} | ${row.namespace} | \`${row.hostPattern}\` | ${row.orgApiVersion} | ${row.offersPinned} | ${row.deploys} / ${row.deployTotal} | ${row.checkOnly} | ${describe(row.statuses)} | ${describe(row.testLevels)} | ${row.componentsSeen} | ${row.componentAuthors} | ${row.coverage} |`,
    ),
  ].join('\n');
...
function document(table, rows) {
  ...
  const scratchOrgs = rows.filter((row) => row.kind === 'scratch').length;
  const truncated = rows.filter((row) => Number(row.deployTotal) > row.deploys).length;
  const checkOnly = rows.reduce((sum, row) => sum + row.checkOnly, 0);
  const apiVersions = [...new Set(rows.map((row) => row.orgApiVersion))].sort().join(' and ');
...
${checkOnly} of the deploys read are check-only runs: Salesforce validated the
...
**${truncated} of these orgs have more deployments than a refresh reads.** The
...
**${scratchOrgs} scratch orgs were invisible to this report until now.** Scratch
...
**Orgs run API ${apiVersions}; all of them still serve 62.0**, the version this
